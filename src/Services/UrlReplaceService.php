<?php
/**
 * Site-wide URL replace: live front-end rules + serialized-safe DB replace.
 *
...
/**
 * Class UrlReplaceService
 */
class UrlReplaceService {

	public const OPTION_RULES = 'seo_campaign_hub_url_replacements';
	...
	public function filter_html( string $html ): string {
		...
		foreach ( $this->get_enabled_pairs() as $pair ) {
			$html = str_replace( $pair['from'], $pair['to'], $html );
			$from_amp = str_replace( '&', '&amp;', $pair['from'] );
			$to_amp   = str_replace( '&', '&amp;', $pair['to'] );
			if ( $from_amp !== $pair['from'] ) {
				$html = str_replace( $from_amp, $to_amp, $html );
			}
		}
		return $html;
	}
	...
	public function add_rule( string $from, string $to ): array {
		...
		return [ 'ok' => true, 'message' => __( 'Live URL rule saved. It applies on the public site immediately.', 'seo-campaign-hub' ) ];
	}
	...
	/**
	 * Dry-run or apply a database-wide replace.
	 *
	 * @param string $from   Find.
	 * @param string $to     Replace.
	...
	public function replace_in_database( string $from, string $to, bool $dry ): array {
		...
		global $wpdb;
		$counts = [
			'posts'     => $this->replace_table_columns( $wpdb->posts, 'ID', [ 'post_content', 'post_excerpt' ], $from, $to, $dry ),
			'postmeta'  => $this->replace_table_columns( $wpdb->postmeta, 'meta_id', [ 'meta_value' ], $from, $to, $dry ),
			'options'   => $this->replace_options( $from, $to, $dry ),
			'comments'  => $this->replace_table_columns( $wpdb->comments, 'comment_ID', [ 'comment_content', 'comment_author_url' ], $from, $to, $dry ),
	...
	public function validate_pair( string $from, string $to ): string {
		if ( strlen( $from ) < 8 || strlen( $to ) < 4 ) {
			return __( 'Find must be at least 8 characters; replace at least 4.', 'seo-campaign-hub' );
		}
		if ( $from === $to ) {
			return __( 'Find and replace URLs must be different.', 'seo-campaign-hub' );
		}
	...
	private function replace_short_links( string $from, string $to, bool $dry ): int {
		...
		return $this->replace_table_columns( $table, 'id', [ 'destination_url', 'targeting_rules' ], $from, $to, $dry );
	}
